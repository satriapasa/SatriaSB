...
name = raw_input("Login using authtoken or qr? WARNING: QR DOES NOT WORK ANYMORE")
...
def bot(op):
	...
	if op.type == 26:
		...
		if "!STFU" == msg.text :
			if msg.to in autoreadpoint:
				...
				cl.sendText(msg.to,"Done. type 'autoreadpoint' to reenable.")
			else:
			...
			else: cl.sendText(msg.to,"No readPoint. create one by typing 'createreadpoint'.")
		elif "!createreadpoint" == msg.text:
			readpoint.append(msg.to)
			readMember[msg.to] = ''
			cl.sendText(msg.to,"Done. type 'showread' to show who's reading.")
		elif "!autoreadpoint" == msg.text:
			if msg.to in autoreadpoint:
				cl.sendText(msg.to,"Already enabled.")
			else:
				autoreadpoint.append(msg.to)
				cl.sendText(msg.to,"Done. type 'createreadpoint' to create a new readpoint, type 'STFU' to disable.")
		elif "!ginfo" == msg.text:
                	if msg.toType == 2:
			...
			cl.sendText(msg.to,"!createreadpoint = create a new read point\n
	...
	elif op.type == 13: #ada yang invite bot ke group
		if mid in op.param3:
